...
class OrSolver(Solver):
	def __init__(self, orSolverType):
		...
		self.solver: pywraplp.Solver = pywraplp.Solver.CreateSolver('', solver_id=orSolverType.value)
	...
	def __atmost(self, lits, weights, bound, boolLit = 0):
		...
		self.cntConstraints += 1
		logging.debug("Constraint #{:d}:   {} <= {}".format(self.cntConstraints, "+".join(["{}*{}".format(weights[i], lits[i]) for i in range(len(lits))]), bound))

	def solve(self):
		res = self.solver.Solve()
		if res == self.solver.OPTIMAL or res == self.solver.FEASIBLE:
			return True
		elif res == self.solver.NOT_SOLVED or res == self.solver.INFEASIBLE:
			return False
